...
def impute_nostop_transit_link_stops(ft, pathset_links_df, veh_trips_df, prev_next):
    ...
    fasttrips.FastTripsLogger.debug(
        "impute_nostop_transit_link_stops: veh_trips_df head=\n%s" % str(veh_trips_df.head()))

    prevnext_links_df = pathset_links_df[
        ["person_id", "person_trip_id", "iteration", "pathfinding_iteration", "simulation_iteration", "pathnum",
         "linknum", "linkmode",
    ...
    pathset_links_df = pd.merge(left=pathset_links_df,
                                ...
                                how="left",
                                on=["person_id", "person_trip_id", "iteration", "pathfinding_iteration",
                                    "simulation_iteration", "pathnum", "linknum"],
    ...
    if prev_next == "prev":
        ...
        find_near_df = pathset_links_df.loc[
            ((pathset_links_df["linkmode_prev"] == "access") | (pathset_links_df["linkmode_prev"] == "transfer")) &
        ...
        find_near_df = pathset_links_df.loc[
            ((pathset_links_df["linkmode_next"] == "egress") | (pathset_links_df["linkmode_next"] == "transfer")) &
    ...
    for service_id in service_id_list:
        ...
        fasttrips.FastTripsLogger.debug(
            "Processing service_id [%s] with %d possible imputes" % (service_id, len(impute_id)))

        service_vehicle_stops = veh_trips_df.loc[
            veh_trips_df["service_id"] == service_id, ["service_id", "mode", "stop_id", "stop_id_num", "stop_name",
        ...
        fasttrips.FastTripsLogger.debug("service_vehicle_stops len=%d head()=\n%s" % (
            len(service_vehicle_stops), str(service_vehicle_stops.head())))
        ...
        if pd.notnull(near_stops_df["stop_id"]).sum() == 0:
            ...
            fasttrips.FastTripsLogger.debug(
                "near_stops_df len=%d head()=\n%s" % (len(near_stops_df), str(near_stops_df.head())))

            # pick the closest one
            near_stops_df = near_stops_df.loc[near_stops_df.groupby(
                ["person_id", "person_trip_id", "iteration", "pathfinding_iteration", "simulation_iteration", "pathnum",
                 "linknum"])["stop_dist"].idxmin(),
                                              ["person_id", "person_trip_id", "iteration", "pathfinding_iteration",
                                               "simulation_iteration", "pathnum", "linknum", "stop_id", "stop_id_num",
                                               "stop_name", "stop_lat", "stop_lon", "stop_dist"]]
            ...
            fasttrips.FastTripsLogger.debug("pathset_links_df head=\n%s" % str(
                pathset_links_df.loc[pd.notnull(pathset_links_df["stop_id"])].head()))
            # set it
            fasttrips.FastTripsLogger.info("Imputing %6d out of %6d stops for %s" % (
                pd.notnull(pathset_links_df["stop_id"]).sum(), len(impute_id), service_id))
            ...
            pathset_links_df.loc[pd.notnull(pathset_links_df["stop_id"]), "%s_id_num" % AB] = pathset_links_df[
                "stop_id_num"]
            ...
            pathset_links_df.drop(["stop_id", "stop_id_num", "stop_name", "stop_lat", "stop_lon", "stop_dist"], axis=1,
                                  inplace=True)
    ...
    pathset_links_df.drop(["linkmode_%s" % prev_next,
                           "A_id_%s" % prev_next, "A_id_num_%s" % prev_next, "A_lat_%s" % prev_next,
                           "A_lon_%s" % prev_next,
                           "B_id_%s" % prev_next, "B_id_num_%s" % prev_next, "B_lat_%s" % prev_next,
                           "B_lon_%s" % prev_next], axis=1, inplace=True)
    return pathset_links_df
...
def impute_stop_from_adjacent_stop(ft, pathset_links_df, prev_next):
    ...
    # inpute the AB_set
    impute_count = len(pathset_links_df.loc[pd.isnull(pathset_links_df["%s_id" % AB_set]) & pd.notnull(
        pathset_links_df["%s_id_%s" % (AB_use, prev_next)])])
    pathset_links_df.loc[pd.isnull(pathset_links_df["%s_id" % AB_set]) & pd.notnull(
        pathset_links_df["%s_id_%s" % (AB_use, prev_next)]), "%s_impute" % AB_set] = True
    pathset_links_df.loc[pd.isnull(pathset_links_df["%s_id" % AB_set]) & pd.notnull(
        pathset_links_df["%s_id_%s" % (AB_use, prev_next)]), "%s_id_num" % AB_set] = pathset_links_df[
        "%s_id_num_%s" % (AB_use, prev_next)]
    pathset_links_df.loc[pd.isnull(pathset_links_df["%s_id" % AB_set]) & pd.notnull(
        pathset_links_df["%s_id_%s" % (AB_use, prev_next)]), "%s_lat" % AB_set] = pathset_links_df[
        "%s_lat_%s" % (AB_use, prev_next)]
    pathset_links_df.loc[pd.isnull(pathset_links_df["%s_id" % AB_set]) & pd.notnull(
        pathset_links_df["%s_id_%s" % (AB_use, prev_next)]), "%s_lon" % AB_set] = pathset_links_df[
        "%s_lon_%s" % (AB_use, prev_next)]
    pathset_links_df.loc[pd.isnull(pathset_links_df["%s_id" % AB_set]) & pd.notnull(
        pathset_links_df["%s_id_%s" % (AB_use, prev_next)]), "%s_id" % AB_set] = pathset_links_df[
        "%s_id_%s" % (AB_use, prev_next)]
    # done
    ...
    fasttrips.FastTripsLogger.info("Imputed %8d values for %s_id => Have %8d null %s_id values" % (
        impute_count, AB_set, pd.isnull(pathset_links_df["%s_id" % AB_set]).sum(), AB_set))
    return pathset_links_df
...
if __name__ == "__main__":
    ...
    (pathset_paths_df, pathset_links_df) = ft.passengers.read_passenger_pathsets(args.input_path_dir[0], ft.stops,
                                                                                 ft.routes.modes_df)
    ...
    veh_trips_df = ft.stops.add_stop_lat_lon(veh_trips_df, id_colname="stop_id", new_lat_colname="stop_lat",
                                             new_lon_colname="stop_lon", new_stop_name_colname="stop_name")
    ...
    fasttrips.FastTripsLogger.info(
        "Splitting pathset_links_df (%d) into links with trip_id (%d) and links without (%d)" %
    ...
    map_link_fields = ["person_id", "person_trip_id", "iteration", "pathfinding_iteration", "simulation_iteration",
                       "pathnum",
    ...
    pathset_paths_df = pathset_paths_df[
        ["person_id", "person_trip_id", "iteration", "pathfinding_iteration", "simulation_iteration", "pathnum",
         "description"]]
    ...
    map_points_df["point_id"] = map_points_df.groupby(
        ["person_id", "person_trip_id", "iteration", "pathfinding_iteration", "simulation_iteration", "pathnum",
         "linknum"]).cumcount() + 1
    ...
    last_point_df = pathset_links_df.groupby(
        ["person_id", "person_trip_id", "iteration", "pathfinding_iteration", "simulation_iteration", "pathnum",
         "linknum"]).agg(
        {"point_id": "count",
         "B_id": "last",
    ...
    map_points_df.sort_values(
        by=["person_id", "person_trip_id", "iteration", "pathfinding_iteration", "simulation_iteration", "pathnum",
            "linknum", "point_id"], inplace=True)
    ...
    if args.use_description:
        ...
        fasttrips.FastTripsLogger.info(
            "Dropping duplicate path descriptions.  Went from %d to %d paths" % (num_paths, len(pathset_paths_df)))

        # join map points to them
        map_points_df = pd.merge(left=pathset_paths_df,
                                 left_on=["person_id", "person_trip_id", "iteration", "pathfinding_iteration",
                                          "simulation_iteration", "pathnum"],
                                 right=map_points_df,
                                 right_on=["person_id", "person_trip_id", "iteration", "pathfinding_iteration",
                                           "simulation_iteration", "pathnum"],
                                 how="inner")
        fasttrips.FastTripsLogger.info("Have %d map points" % len(map_points_df))
        ...
        map_points_df = pd.merge(left=map_points_df,
                                 ...
                                 right=pathset_paths_df[
                                     ["person_id", "person_trip_id", "iteration", "pathfinding_iteration",
                                      "simulation_iteration", "pathnum", "description"]],
                                 right_on=["person_id", "person_trip_id", "iteration", "pathfinding_iteration",
                                           "simulation_iteration", "pathnum"],
                                 how="left")
    # write it
