...
def compare_file(dir1, dir2, filename):
    ...
    for colname in list(df1.columns.values):
        ...
        if df_diff[col1].dtype != df_diff[col2].dtype:
            FastTripsLogger.debug("mismatching dtypes for %s and %s: %s vs %s" % (
            col1, col2, str(df_diff[col1].dtype), str(df_diff[col2].dtype)))
            df_diff[coldiff] = 0
        ...
        if df_diff[colabsdiff].max() == 0:
            ...
            FastTripsLogger.debug(" -- diffs --\n" + \
                                  str(df_diff.reset_index().sort(columns=[colabsdiff, 'trip_list_id_num'],
                                                                 ascending=[False, True]).loc[:,
                                      ['trip_list_id_num', 'mode', 'originTaz', 'destinationTaz', col1, col2, coldiff,
                                       colabsdiff]].head()) + "\n")
        else:
            FastTripsLogger.debug(" -- diffs --\n" + \
                                  str(df_diff.sort(columns=colabsdiff, ascending=False).loc[:,
                                      [col1, col2, coldiff, colabsdiff]].head()) + "\n")
        ...
        if df_diff[colabsdiff].max() == 0:
            ...
            status = "Values differ by [%s,%s] mins with mean %s" % (
            str(df_diff[coldiff].min()), str(df_diff[coldiff].max()), str(df_diff[coldiff].mean()))
        else:
            status = "Values differ by [% 8.2f,% 8.2f] with mean % 8.2f" % (
            df_diff[coldiff].min(), df_diff[coldiff].max(), df_diff[coldiff].mean())
...
def compare_pathset(dir1, dir2):
    ...
    merge_cols = ['iteration', 'passenger_id_num', 'trip_list_id_num', 'path_board_stops', 'path_trips',
                  'path_alight_stops']
    ...
    df_diff_counts = df_diff.groupby(['iteration', 'passenger_id_num', 'trip_list_id_num']).agg(
        {'num total paths': 'count', 'exp_util': 'sum'})
    ...
    df_diff = df_diff.merge(
        df_diff_counts.reset_index()[['iteration', 'passenger_id_num', 'trip_list_id_num', 'sum_exp_util']], how='left')
    ...
    # look at the nulls
    df1_only = df_diff.loc[pd.isnull(df_diff.path_cost_2)].groupby(
        ['iteration', 'passenger_id_num', 'trip_list_id_num']).agg(
        {'union pathset probability': 'max', 'path_cost_1': 'count'})
    ...
    df2_only = df_diff.loc[pd.isnull(df_diff.path_cost_1)].groupby(
        ['iteration', 'passenger_id_num', 'trip_list_id_num']).agg(
        {'union pathset probability': 'max', 'path_cost_2': 'count'})
    ...
    df_diff_summary.loc[
        df_diff_summary['num paths missing from file2'] == df_diff_summary['num total paths'], 'only in file1'] = 1
    df_diff_summary['only in file2'] = 0
    df_diff_summary.loc[
        df_diff_summary['num paths missing from file1'] == df_diff_summary['num total paths'], 'only in file2'] = 1
    # NaN means zero
    ...
    FastTripsLogger.info(
        "                        Average pathset size: %.1f" % df_diff_summary['num total paths'].mean())
    ...
    FastTripsLogger.info(
        "        Average paths missing from pathset 1: %.1f" % df_diff_summary['num paths missing from file1'].mean())
    FastTripsLogger.info("      Max probability missing from pathset 1: %.3f%%" % (
                100 * df_diff_summary['max prob missing from file1'].max()))
    FastTripsLogger.info(" # trips w/ paths>10%% missing from pathset 1: %d" % len(
        df_diff_summary.loc[df_diff_summary['max prob missing from file1'] > 0.10]))
    FastTripsLogger.info(" # trips w/ paths> 1%% missing from pathset 1: %d" % len(
        df_diff_summary.loc[df_diff_summary['max prob missing from file1'] > 0.01]))
    temp_df = df_diff_summary[
        ['max prob missing from file1', 'num paths missing from file1']].reset_index().sort_values(
        by=['max prob missing from file1', 'trip_list_id_num'], ascending=[False, True])
    FastTripsLogger.debug(" trips w/ paths> 1%% missing from pathset 1\n%s\n" %
                          str(temp_df.loc[temp_df['max prob missing from file1'] > 0.01]))

    FastTripsLogger.info(
        "        Average paths missing from pathset 2: %.1f" % df_diff_summary['num paths missing from file2'].mean())
    FastTripsLogger.info("      Max probability missing from pathset 2: %.3f%%" % (
                100 * df_diff_summary['max prob missing from file2'].max()))
    FastTripsLogger.info(" # trips w/ paths>10%% missing from pathset 2: %d" % len(
        df_diff_summary.loc[df_diff_summary['max prob missing from file2'] > 0.10]))
    FastTripsLogger.info(" # trips w/ paths> 1%% missing from pathset 2: %d" % len(
        df_diff_summary.loc[df_diff_summary['max prob missing from file2'] > 0.01]))
    temp_df = df_diff_summary[
        ['max prob missing from file2', 'num paths missing from file2']].reset_index().sort_values(
        by=['max prob missing from file2', 'trip_list_id_num'], ascending=[False, True])
...
def compare_performance(dir1, dir2):
    ...
    df1.drop([fasttrips.Performance.PERFORMANCE_COLUMN_TIME_LABELING,
              fasttrips.Performance.PERFORMANCE_COLUMN_TIME_ENUMERATING], axis=1, inplace=True)
    df2.drop([fasttrips.Performance.PERFORMANCE_COLUMN_TIME_LABELING,
              fasttrips.Performance.PERFORMANCE_COLUMN_TIME_ENUMERATING], axis=1, inplace=True)
