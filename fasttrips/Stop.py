...
class Stop(object):
    ...
    def __init__(self, input_archive, output_dir, gtfs, today):
        ...
        if Stop.STOPS_COLUMN_ZONE_ID in list(self.stops_df.columns.values):
            ...
            if len(zones_df) > 0:
                ...
                # add it to the stops
                self.stops_df = pd.merge(left=self.stops_df, right=self.zone_id_df, how="left",
                                         on=Stop.STOPS_COLUMN_ZONE_ID)
    ...
    def add_trips(self, stop_times_df):
        ...
        self.trip_times_df.set_index(
            [Trip.STOPTIMES_COLUMN_STOP_ID, Trip.STOPTIMES_COLUMN_TRIP_ID, Trip.STOPTIMES_COLUMN_SEQUENCE],
    ...
    def get_trips_arriving_within_time(self, stop_id, latest_arrival, time_window):
        ...
        df = df.loc[(df[Trip.STOPTIMES_COLUMN_ARRIVAL_TIME_MIN] < latest_arrival_min) &
                    (df[Trip.STOPTIMES_COLUMN_ARRIVAL_TIME_MIN] > (
                                latest_arrival_min - (time_window.total_seconds() / 60.0)))]
    ...
    def get_trips_departing_within_time(self, stop_id, earliest_departure, time_window):
        ...
        """
        earliest_departure_min = 60.0 * earliest_departure.hour + earliest_departure.minute + (
                    earliest_departure.second / 60.0)
        # filter to stop
        df = self.trip_times_df.loc[stop_id]
        # depart after the earliest departure
        df = df.loc[(df[Trip.STOPTIMES_COLUMN_DEPARTURE_TIME_MIN] > earliest_departure_min) &
                    (df[Trip.STOPTIMES_COLUMN_DEPARTURE_TIME_MIN] < (
                                earliest_departure_min + (time_window.total_seconds() / 60.0)))]
    ...
    def is_transfer(self, stop_id, xfer_from):
        ...
        if xfer_from and len(self.transfers_df.loc[self.transfers_df[Stop.TRANSFERS_COLUMN_FROM_STOP] == stop_id]) > 0:
            return True
        if not xfer_from and len(
                self.transfers_df.loc[self.transfers_df[Stop.TRANSFERS_COLUMN_TO_STOP] == stop_id]) > 0:
            return True
        return False
