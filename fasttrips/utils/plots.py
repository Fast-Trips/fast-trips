...
def plot_demand_time(demand_dir, aggregation_level=20):
    triplist_df = pd.read_csv(os.path.join(demand_dir, 'trip_list.txt'),
                              usecols=['departure_time', 'arrival_time', 'person_id', 'time_target'])
    triplist_df["freq"] = 1
    ...
    depart_agg = triplist_df[triplist_df['time_target'] == 'departure'].resample('%dT' % (aggregation_level),
                                                                                 on='departure_time_dt').sum().rename(
        columns={'freq': 'trips_depart'})
    arrive_agg = triplist_df[triplist_df['time_target'] == 'arrival'].resample('%dT' % (aggregation_level),
                                                                               on='arrival_time_dt').sum().rename(
        columns={'freq': 'trips_arrive'})

    p = figure(plot_height=350, x_axis_type="datetime", toolbar_location=None, tools="",
               title="Time of Day Distribution of Transit Trips in Demand", width=800)
    p.xaxis.axis_label = "Time of Day"
    p.yaxis.axis_label = "Number of trips, aggregated to %d minutes" % (aggregation_level)
    p.line(x=list(depart_agg.index), y=list(depart_agg['trips_depart']), legend="departures", color="green",
           line_width=2)
    p.line(x=list(arrive_agg.index), y=list(arrive_agg['trips_arrive']), legend="arrivals", color="blue", line_width=2)

    p.vbar(x=[x for x in list(depart_agg.index)], width=aggregation_level / 2, bottom=0,
           top=list(depart_agg['trips_depart']), color="green")
    p.vbar(x=[x for x in list(arrive_agg.index)], width=aggregation_level / 2, bottom=0,
           top=list(arrive_agg['trips_arrive']), color="blue")
    return p


def plot_route_timing(network_dir, sel_routes=[]):
    stoptimes_df = pd.read_csv(os.path.join(network_dir, 'stop_times.txt'),
    ...
    firststop_df.loc[firststop_df['direction_id'] == 0, 'route_id-dir'] = firststop_df.loc[firststop_df[
                                                                                               'direction_id'] == 0, 'route_id'] + "-Out"
    firststop_df.loc[firststop_df['direction_id'] == 1, 'route_id-dir'] = firststop_df.loc[firststop_df[
                                                                                               'direction_id'] == 1, 'route_id'] + "-In"
