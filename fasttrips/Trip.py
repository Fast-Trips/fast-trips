...
class Trip(object):
    ...
    def __init__(self, input_archive, output_dir, gtfs_feed, today, stops, routes, prepend_route_id_to_trip_id):
        ...
        if prepend_route_id_to_trip_id:
            ...
            trip_id_df = pd.merge(self.trip_id_df,
                                  self.trips_df[[Trip.TRIPS_COLUMN_TRIP_ID, Trip.TRIPS_COLUMN_ROUTE_ID]],
                                  how='left', on=Trip.TRIPS_COLUMN_TRIP_ID)
            trip_id_df.rename(columns={Trip.TRIPS_COLUMN_TRIP_ID: 'trip_id_orig'}, inplace=True)
            trip_id_df[Trip.TRIPS_COLUMN_TRIP_ID] = trip_id_df[Trip.TRIPS_COLUMN_ROUTE_ID].map(str) + str("_") + \
                                                    trip_id_df['trip_id_orig']
        else:
        ...
        self.stop_times_df[Trip.STOPTIMES_COLUMN_ARRIVAL_TIME] = self.stop_times_df[
                                                                     Trip.STOPTIMES_COLUMN_ARRIVAL_TIME] / 60
        self.stop_times_df[Trip.STOPTIMES_COLUMN_DEPARTURE_TIME] = self.stop_times_df[
                                                                       Trip.STOPTIMES_COLUMN_DEPARTURE_TIME] / 60
    ...
    def add_original_travel_time_and_dwell(self):
        ...
        # copy
        self.stop_times_df[Trip.STOPTIMES_COLUMN_TRAVEL_TIME] = self.stop_times_df[
            Trip.STOPTIMES_COLUMN_ORIGINAL_TRAVEL_TIME]
    ...
    def add_trip_attrs_from_stoptimes(self):
        ...
        stops_by_trip.rename(columns={Trip.STOPTIMES_COLUMN_STOP_SEQUENCE: Trip.TRIPS_COLUMN_MAX_STOP_SEQUENCE,
                                      Trip.STOPTIMES_COLUMN_DEPARTURE_TIME: Trip.TRIPS_COLUMN_TRIP_DEPARTURE_TIME},
                             inplace=True)
    ...
    def add_shape_dist_traveled(self, stops):
        ...
        self.stop_times_df["null_shape_dist_traveled"] = pd.isnull(
            self.stop_times_df[Trip.STOPTIMES_COLUMN_SHAPE_DIST_TRAVELED])
        FastTripsLogger.debug(
            "add_shape_dist_traveled: missing %d out of %d shape_dist_traveled values in the stop times dataframe" %
        ...
        stop_times_trips = self.stop_times_df[[Trip.STOPTIMES_COLUMN_TRIP_ID, "null_shape_dist_traveled"]].groupby(
            [Trip.STOPTIMES_COLUMN_TRIP_ID]).sum()
        ...
        self.stop_times_df = stops.add_stop_lat_lon(self.stop_times_df, Trip.STOPTIMES_COLUMN_STOP_ID, "stop_lat",
                                                    "stop_lon")
        ...
        self.stop_times_df = pd.merge(left=self.stop_times_df,
                                      ...
                                      how="left",
                                      suffixes=["", "_prev"])
        Util.calculate_distance_miles(self.stop_times_df, "stop_lat", "stop_lon", "stop_lat_prev", "stop_lon_prev",
                                      "calc shape_dist_traveled")

        # make it cumulative
        self.stop_times_df.loc[(self.stop_times_df["null_shape_dist_traveled_bool"]) &
                               (self.stop_times_df[
                                    Trip.STOPTIMES_COLUMN_STOP_SEQUENCE] == 1), "calc shape_dist_traveled"] = 0.0
        self.stop_times_df["calc shape_dist_traveled"] = self.stop_times_df.groupby([Trip.STOPTIMES_COLUMN_TRIP_ID])[
            "calc shape_dist_traveled"].apply(lambda x: x.cumsum())

        # incorporate it
        self.stop_times_df.loc[
            (self.stop_times_df["null_shape_dist_traveled_bool"]), Trip.STOPTIMES_COLUMN_SHAPE_DIST_TRAVELED] = \
            self.stop_times_df["calc shape_dist_traveled"]
    ...
    def update_trip_times(trips_df, MSA_RESULTS):
        ...
        if Trip.VEHICLES_COLUMN_SEATED_CAPACITY in trip_cols:
            ...
            if pd.isnull(trips_df[Trip.VEHICLES_COLUMN_SEATED_CAPACITY]).sum() > 0:
                FastTripsLogger.warn(
                    "Trip.update_trip_times(): some [%s] not configured; assuming zero friction for those vehicles" % Trip.VEHICLES_COLUMN_SEATED_CAPACITY)
                FastTripsLogger.warn(
                    "\n%s" % trips_df[[Trip.VEHICLES_COLUMN_VEHICLE_NAME, Trip.VEHICLES_COLUMN_SEATED_CAPACITY]].loc[
                        pd.isnull(trips_df[Trip.VEHICLES_COLUMN_SEATED_CAPACITY])].drop_duplicates())

            # set standeeds
            trips_df[Trip.SIM_COL_VEH_STANDEES] = trips_df[Trip.SIM_COL_VEH_ONBOARD] - trips_df[
                Trip.VEHICLES_COLUMN_SEATED_CAPACITY]
            trips_df[Trip.SIM_COL_VEH_MSA_STANDEES] = trips_df[Trip.SIM_COL_VEH_MSA_ONBOARD] - trips_df[
                Trip.VEHICLES_COLUMN_SEATED_CAPACITY]
            ...
            trips_df.loc[(trips_df[Trip.SIM_COL_VEH_STANDEES] > 0) & (
                pd.notnull(trips_df[Trip.VEHICLES_COLUMN_SEATED_CAPACITY])), Trip.SIM_COL_VEH_FRICTION] = \
                trips_df[Trip.SIM_COL_VEH_BOARDS] + trips_df[Trip.SIM_COL_VEH_ALIGHTS] + trips_df[
                    Trip.SIM_COL_VEH_STANDEES]
            trips_df.loc[(trips_df[Trip.SIM_COL_VEH_MSA_STANDEES] > 0) & (
                pd.notnull(trips_df[Trip.VEHICLES_COLUMN_SEATED_CAPACITY])), Trip.SIM_COL_VEH_MSA_FRICTION] = \
                trips_df[Trip.SIM_COL_VEH_MSA_BOARDS] + trips_df[Trip.SIM_COL_VEH_MSA_ALIGHTS] + trips_df[
                    Trip.SIM_COL_VEH_MSA_STANDEES]
        else:
            # log no seated capacities at all
            FastTripsLogger.warn(
                "Trip.update_trip_times(): Cannot calculate friction because [%s] not configured" % Trip.VEHICLES_COLUMN_SEATED_CAPACITY)
        ...
        if Trip.VEHICLES_COLUMN_DWELL_FORMULA in trip_cols:
            ...
            # keep the dwell time
            trips_df[Trip.STOPTIMES_COLUMN_DWELL_TIME] = trips_df[Trip.STOPTIMES_COLUMN_DWELL_TIME_SEC].map(
        ...
        next_stop_df = trips_df[[Trip.STOPTIMES_COLUMN_TRIP_ID_NUM,
                                 ...
                                 "does_stop"]].copy()
        next_stop_df.loc[:, "is_last_stop"] = next_stop_df[Trip.STOPTIMES_COLUMN_STOP_SEQUENCE] == next_stop_df[
            Trip.TRIPS_COLUMN_MAX_STOP_SEQUENCE]
        next_stop_df.loc[:, Trip.STOPTIMES_COLUMN_STOP_SEQUENCE] = next_stop_df[Trip.STOPTIMES_COLUMN_STOP_SEQUENCE] - 1
        ...
        if (Trip.VEHICLES_COLUMN_MAXIMUM_SPEED_FPS in trip_cols) and \
            ...
            trips_df.loc[trips_df["does_stop"] & \
                         ...
                         (trips_df[Trip.STOPTIMES_COLUMN_STOP_SEQUENCE] < trips_df[
                             Trip.TRIPS_COLUMN_MAX_STOP_SEQUENCE]), "accel_secs"] = \
        ...
        trips_df[Trip.STOPTIMES_COLUMN_TRAVEL_TIME_SEC] = trips_df[
                                                              Trip.STOPTIMES_COLUMN_ORIGINAL_TRAVEL_TIME] / np.timedelta64(
            1, 's')
        trips_df.loc[pd.notnull(trips_df["accel_secs"]), Trip.STOPTIMES_COLUMN_TRAVEL_TIME_SEC] = trips_df[
                                                                                                      Trip.STOPTIMES_COLUMN_TRAVEL_TIME_SEC] + \
                                                                                                  trips_df["accel_secs"]
        trips_df.loc[pd.notnull(trips_df["decel_secs"]), Trip.STOPTIMES_COLUMN_TRAVEL_TIME_SEC] = trips_df[
                                                                                                      Trip.STOPTIMES_COLUMN_TRAVEL_TIME_SEC] + \
                                                                                                  trips_df["decel_secs"]

        trips_df[Trip.STOPTIMES_COLUMN_TRAVEL_TIME] = trips_df[Trip.STOPTIMES_COLUMN_TRAVEL_TIME_SEC].map(
            lambda x: datetime.timedelta(seconds=x))

        # put travel time + dwell together because that's the full time for a link (stop arrival time to next stop arrival time)
        trips_df["travel_dwell_sec"] = trips_df[Trip.STOPTIMES_COLUMN_TRAVEL_TIME_SEC] + trips_df[
            Trip.STOPTIMES_COLUMN_DWELL_TIME_SEC]
        # cumulatively sum it to get arrival times times for the trip
        trips_df["travel_dwell_sec_cum"] = trips_df.groupby([Trip.STOPTIMES_COLUMN_TRIP_ID_NUM])[
            "travel_dwell_sec"].cumsum()
        trips_df["travel_dwell_cum"] = trips_df["travel_dwell_sec_cum"].map(
            lambda x: datetime.timedelta(seconds=x) if pd.notnull(x) else None)
        ...
        first_dwell_df = trips_df[[Trip.STOPTIMES_COLUMN_TRIP_ID_NUM, Trip.STOPTIMES_COLUMN_DWELL_TIME_SEC]]. \
            groupby([Trip.STOPTIMES_COLUMN_TRIP_ID_NUM]).agg(
            {Trip.STOPTIMES_COLUMN_DWELL_TIME_SEC: 'first'}).reset_index()
        first_dwell_df.rename(columns={Trip.STOPTIMES_COLUMN_DWELL_TIME_SEC: "trip_first_dwell_sec"}, inplace=True)
        first_dwell_df["trip_first_dwell"] = first_dwell_df["trip_first_dwell_sec"].map(
            lambda x: datetime.timedelta(seconds=x))
        ...
        next_stop_df["trip_arrival_time"] = next_stop_df[Trip.TRIPS_COLUMN_TRIP_DEPARTURE_TIME] - next_stop_df[
            "trip_first_dwell"]
        ...
        trips_df.loc[pd.notnull(trips_df["new_arrival_time"]), Trip.STOPTIMES_COLUMN_ARRIVAL_TIME] = trips_df[
            "new_arrival_time"]
        # set the first ones to be departure time minus dwell time
        trips_df.loc[pd.isnull(trips_df["new_arrival_time"]), Trip.STOPTIMES_COLUMN_ARRIVAL_TIME] = trips_df[
                                                                                                        Trip.STOPTIMES_COLUMN_DEPARTURE_TIME] - \
                                                                                                    trips_df[
                                                                                                        Trip.STOPTIMES_COLUMN_DWELL_TIME]
        # departure time is arrival time + dwell
        trips_df.loc[:, Trip.STOPTIMES_COLUMN_DEPARTURE_TIME] = trips_df[Trip.STOPTIMES_COLUMN_ARRIVAL_TIME] + trips_df[
            Trip.STOPTIMES_COLUMN_DWELL_TIME]
        ...
        FastTripsLogger.debug("Trips:update_trip_times() trips_df:\n%s\n" % \
                              trips_df.loc[
                                  trips_df[Trip.TRIPS_COLUMN_MAX_STOP_SEQUENCE] > 1, [Trip.STOPTIMES_COLUMN_TRIP_ID,
                                                                                      Trip.STOPTIMES_COLUMN_TRIP_ID_NUM,
                                                                                      Trip.STOPTIMES_COLUMN_STOP_SEQUENCE,
                                                                                      Trip.STOPTIMES_COLUMN_ARRIVAL_TIME,
                                                                                      Trip.STOPTIMES_COLUMN_DEPARTURE_TIME,
                                                                                      Trip.VEHICLES_COLUMN_MAXIMUM_SPEED_FPS,
                                                                                      Trip.VEHICLES_COLUMN_ACCELERATION,
                                                                                      ...
                                                                                      Trip.SIM_COL_VEH_STANDEES,
                                                                                      Trip.SIM_COL_VEH_FRICTION,
                                                                                      "does_stop", "next_does_stop",
                                                                                      "next_is_last_stop",
                                                                                      "accel_secs", "decel_secs",
                                                                                      Trip.STOPTIMES_COLUMN_DWELL_TIME_SEC,
                                                                                      "travel_dwell_sec",
                                                                                      "travel_dwell_sec_cum",
                                                                                      "new_arrival_time"
                                                                                      ]].head(15).to_string())
    ...
    def linkify_vehicle_trips(veh_trips_df, stops):
        ...
        * A_arrival_time, A_departure_time, B_arrival_time, B_departure_time
        ...
        veh_temp_df = stops.add_stop_lat_lon(veh_temp_df, id_colname=Trip.STOPTIMES_COLUMN_STOP_ID,
                                             new_lat_colname="lat", new_lon_colname="lon")
        ...
        veh_temp_df = pd.merge(left=veh_temp_df,
                               ...
                               right_on=[Route.ROUTES_COLUMN_MODE, Trip.TRIPS_COLUMN_MODE_NUM,
                                         Trip.TRIPS_COLUMN_ROUTE_ID, Trip.TRIPS_COLUMN_ROUTE_ID_NUM,
                                         Trip.TRIPS_COLUMN_TRIP_ID, Trip.TRIPS_COLUMN_TRIP_ID_NUM,
                                         Trip.STOPTIMES_COLUMN_STOP_SEQUENCE],
                               how="left",
                               suffixes=["_A", "_B"])
    ...
    def calculate_headways(trips_df):
        ...
        stop_group_df['headway'] = (stop_group_df[Trip.STOPTIMES_COLUMN_DEPARTURE_TIME] - stop_group_shift_df[
            Trip.STOPTIMES_COLUMN_DEPARTURE_TIME]) / np.timedelta64(1, 'm')
