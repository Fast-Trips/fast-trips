...
class Route(object):
    ...
    def __init__(self, input_archive, output_dir, gtfs, today, stops):
        ...
        if routes_ft_df.duplicated(subset=[Route.ROUTES_COLUMN_ROUTE_ID]).sum() > 0:
            error_msg = "Found %d duplicate %s in %s" % (
                routes_ft_df.duplicated(subset=[Route.ROUTES_COLUMN_ROUTE_ID]).sum(),
        ...
        if Assignment.FARE_ZONE_SYMMETRY:
            ...
            reverse_fare_rules = self.fare_rules_df.loc[
            ...
            # reverse them
            reverse_fare_rules.rename(
                columns={Route.FARE_RULES_COLUMN_ORIGIN_ID: Route.FARE_RULES_COLUMN_DESTINATION_ID,
        ...
        if not fare_rules_ft_df.empty:
            ...
            if Route.FARE_RULES_COLUMN_CONTAINS_ID in list(self.fare_rules_df.columns.values):
                non_null_contains_id = self.fare_rules_df.loc[
                    pd.notnull(self.fare_rules_df[Route.FARE_RULES_COLUMN_CONTAINS_ID])]
            ...
            self.fare_rules_df[Route.FARE_RULES_COLUMN_FARE_PERIOD] = self.fare_rules_df[
        ...
        if not self.fare_transfer_rules_df.empty:
            ...
            invalid_type = self.fare_transfer_rules_df.loc[
                self.fare_transfer_rules_df[Route.FARE_TRANSFER_RULES_COLUMN_TYPE].isin(
                    Route.TRANSFER_TYPE_OPTIONS) == False]
            ...
            negative_amount = self.fare_transfer_rules_df.loc[
                self.fare_transfer_rules_df[Route.FARE_TRANSFER_RULES_COLUMN_AMOUNT] < 0]
            ...
            FastTripsLogger.info("Read %7d %15s from %25s" %
                                 (len(self.fare_transfer_rules_df), "fare xfer rules",
    ...
    def write_routes_for_extension(self):
        ...
        if len(self.fare_rules_df) > 0:
            ...
            fare_rules_df[Route.FARE_RULES_COLUMN_START_TIME] = (fare_rules_df[
                                                                     Route.FARE_RULES_COLUMN_START_TIME] - Assignment.NETWORK_BUILD_DATE_START_TIME) / np.timedelta64(
                1, 'm')
            fare_rules_df[Route.FARE_RULES_COLUMN_END_TIME] = (fare_rules_df[
                                                                   Route.FARE_RULES_COLUMN_END_TIME] - Assignment.NETWORK_BUILD_DATE_START_TIME) / np.timedelta64(
                1, 'm')

            # fillna with -1
            for num_col in [Route.FARE_RULES_COLUMN_ROUTE_ID_NUM, Route.FARE_RULES_COLUMN_ORIGIN_ID_NUM,
                            Route.FARE_RULES_COLUMN_DESTINATION_ID_NUM, Route.FARE_ATTR_COLUMN_TRANSFERS]:
            ...
            fare_rules_df["duration"] = fare_rules_df[Route.FARE_RULES_COLUMN_END_TIME] - fare_rules_df[
                Route.FARE_RULES_COLUMN_START_TIME]
            fare_rules_df.sort_values(by=[Route.FARE_RULES_COLUMN_FARE_ID_NUM, "duration"], ascending=True,
                                      inplace=True)
            ...
            FastTripsLogger.debug(
                "No fare rules so no file %s" % os.path.join(self.output_dir, Route.OUTPUT_FARE_ID_FILE))
        ...
        * :py:attr:`Assignment.SIM_COL_PAX_FREE_TRANSFER`
        ...
        if len(fare_rules0) > 0:
            trip_links_match0 = pd.merge(left=trip_links_unmatched,
                                         ...
                                         right_on=[Route.FARE_RULES_COLUMN_ROUTE_ID, Route.FARE_RULES_COLUMN_ORIGIN_ID,
                                                   Route.FARE_RULES_COLUMN_DESTINATION_ID],
            ...
            trip_links_match0 = trip_links_match0.loc[pd.isnull(trip_links_match0[Route.FARE_ATTR_COLUMN_PRICE]) |
                                                      ((trip_links_match0[Assignment.SIM_COL_PAX_BOARD_TIME] >=
                                                        trip_links_match0[Route.FARE_RULES_COLUMN_START_TIME]) &
                                                       (trip_links_match0[Assignment.SIM_COL_PAX_BOARD_TIME] <
                                                        trip_links_match0[Route.FARE_RULES_COLUMN_END_TIME]))]
            FastTripsLogger.debug(
            ...
            if len(trip_links_match0) > 0:
                ...
                FastTripsLogger.debug("matched: %d  unmatched: %d   total: %d" % (
                    len(trip_links_matched), len(trip_links_unmatched),
                    len(trip_links_matched) + len(trip_links_unmatched)))
        ...
        if len(fare_rules1) > 0:
            ...
            trip_links_match1 = trip_links_match1.loc[pd.isnull(trip_links_match1[Route.FARE_ATTR_COLUMN_PRICE]) |
                                                      ((trip_links_match1[Assignment.SIM_COL_PAX_BOARD_TIME] >=
                                                        trip_links_match1[Route.FARE_RULES_COLUMN_START_TIME]) &
                                                       (trip_links_match1[Assignment.SIM_COL_PAX_BOARD_TIME] <
                                                        trip_links_match1[Route.FARE_RULES_COLUMN_END_TIME]))]
            FastTripsLogger.debug(
        ...
        if len(fare_rules2) > 0:
            trip_links_match2 = pd.merge(left=trip_links_unmatched,
                                         ...
                                         right_on=[Route.FARE_RULES_COLUMN_ORIGIN_ID,
                                                   Route.FARE_RULES_COLUMN_DESTINATION_ID],
                                         suffixes=["", "_fare_rules"])

            # delete rows where the board time is not within the fare period
            trip_links_match2 = trip_links_match2.loc[pd.isnull(trip_links_match2[Route.FARE_ATTR_COLUMN_PRICE]) |
                                                      ((trip_links_match2[Assignment.SIM_COL_PAX_BOARD_TIME] >=
                                                        trip_links_match2[Route.FARE_RULES_COLUMN_START_TIME]) &
                                                       (trip_links_match2[Assignment.SIM_COL_PAX_BOARD_TIME] <
                                                        trip_links_match2[Route.FARE_RULES_COLUMN_END_TIME]))]
            FastTripsLogger.debug(
        ...
        fare_rules3 = self.fare_rules_df.loc[pd.isnull(self.fare_rules_df[Route.FARE_RULES_COLUMN_ROUTE_ID]) &
                                             pd.isnull(self.fare_rules_df[Route.FARE_RULES_COLUMN_ORIGIN_ID]) &
                                             pd.isnull(
                                                 self.fare_rules_df[Route.FARE_RULES_COLUMN_DESTINATION_ID])].copy()
        if len(fare_rules3) > 0:
            ...
            trip_links_match3 = trip_links_match3.loc[pd.isnull(trip_links_match3[Route.FARE_ATTR_COLUMN_PRICE]) |
                                                      ((trip_links_match3[Assignment.SIM_COL_PAX_BOARD_TIME] >=
                                                        trip_links_match3[Route.FARE_RULES_COLUMN_START_TIME]) &
                                                       (trip_links_match3[Assignment.SIM_COL_PAX_BOARD_TIME] <
                                                        trip_links_match3[Route.FARE_RULES_COLUMN_END_TIME]))]
            FastTripsLogger.debug(
        ...
        # reorder columns
        trip_links_df = trip_links_df[
            orig_columns + fare_columns + [Route.FARE_ATTR_COLUMN_TRANSFERS, Route.FARE_ATTR_COLUMN_TRANSFER_DURATION]]
    ...
    def apply_fare_transfer_rules(self, trip_links_df):
        ...
        trip_links_df.loc[pd.notnull(trip_links_df[Route.FARE_TRANSFER_RULES_COLUMN_TYPE]) &
                          (trip_links_df[
                               Route.FARE_TRANSFER_RULES_COLUMN_TYPE] == Route.TRANSFER_TYPE_TRANSFER_DISCOUNT),
                          Assignment.SIM_COL_PAX_FARE] = trip_links_df[Assignment.SIM_COL_PAX_FARE] - trip_links_df[
            Route.FARE_TRANSFER_RULES_COLUMN_AMOUNT]
    ...
    def apply_free_transfers(self, trip_links_df):
        ...
        trip_links_df["transfer_time_sec"] = (trip_links_df[Assignment.SIM_COL_PAX_BOARD_TIME] - trip_links_df[
            "%s_ffb" % Assignment.SIM_COL_PAX_BOARD_TIME]) / np.timedelta64(1, 's')
        ...
        trip_links_df.loc[(trip_links_df[Route.FARE_ATTR_COLUMN_TRANSFERS] > 0) &  # transfers > 0
                          ...
                          (trip_links_df["fare_index"] <= trip_links_df[
                              Route.FARE_ATTR_COLUMN_TRANSFERS]),  # is within the number of free transfers allowed
        ...
        trip_links_df.loc[(trip_links_df[Assignment.SIM_COL_PAX_FREE_TRANSFER] == 1.0) &
                          (trip_links_df["transfer_time_sec"] > trip_links_df[
                              Route.FARE_ATTR_COLUMN_TRANSFER_DURATION]),
