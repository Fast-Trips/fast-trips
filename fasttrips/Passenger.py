...
class Passenger(object):
    ...
    def __init__(self, input_dir, output_dir, today, stops, routes, capacity_constraint):
        ...
        if len(missing_ods) > 0:
            FastTripsLogger.warn(
                "Missing origin or destination for the following trips. Dropping.\n%s" % str(missing_ods))
            self.trip_list_df = self.trip_list_df.loc[
                pd.notnull(self.trip_list_df[Passenger.TRIP_LIST_COLUMN_ORIGIN_TAZ_ID]) &
        ...
        if non_zero_person_ids > 0 and os.path.exists(os.path.join(input_dir, Passenger.INPUT_PERSONS_FILE)):
            ...
            self.households_df = pd.read_csv(os.path.join(input_dir, Passenger.INPUT_HOUSEHOLDS_FILE),
                                             skipinitialspace=True)
        ...
        if len(low_vot) > 0:
            FastTripsLogger.warn(
                "These trips have value of time lower than the minimum threshhhold (%f): raising to minimum.\n%s" %
        ...
        if len(self.persons_df) > 0:
            ...
            if len(no_person_ids) > 0:
                error_msg = "Even though a person list is given, failed to find person information for %d trips" % len(
                    no_person_ids)
                FastTripsLogger.fatal(error_msg)
        ...
        bad_mode_df = self.trip_list_df.loc[((self.trip_list_df['mode_dash_count'] != 2) &
                                             ((self.trip_list_df['mode_dash_count'] != 0) |
                                              (self.trip_list_df[
                                                   Passenger.TRIP_LIST_COLUMN_MODE] != Passenger.MODE_GENERIC_TRANSIT)))]
        ...
        self.trip_list_df.loc[self.trip_list_df['mode_dash_count'] == 2,
                              Passenger.TRIP_LIST_COLUMN_ACCESS_MODE] = self.trip_list_df[
            Passenger.TRIP_LIST_COLUMN_MODE] \
            .map(lambda x: "%s" % x[:x.find('-')])
        self.trip_list_df.loc[self.trip_list_df['mode_dash_count'] == 2,
                              Passenger.TRIP_LIST_COLUMN_TRANSIT_MODE] = self.trip_list_df[
            Passenger.TRIP_LIST_COLUMN_MODE] \
            .map(lambda x: x[x.find('-') + 1:x.rfind('-')])
        self.trip_list_df.loc[self.trip_list_df['mode_dash_count'] == 2,
                              Passenger.TRIP_LIST_COLUMN_EGRESS_MODE] = self.trip_list_df[
            Passenger.TRIP_LIST_COLUMN_MODE] \
        ...
        if len(invalid_time_target) > 0:
            ...
            Passenger.TRIP_LIST_COLUMN_TIME_TARGET, str(invalid_time_target))
            FastTripsLogger.fatal(error_msg)
            raise DemandInputError(Passenger.INPUT_TRIP_LIST_FILE, error_msg)

        # set outbound
        self.trip_list_df[Passenger.TRIP_LIST_COLUMN_OUTBOUND] = (
                    self.trip_list_df[Passenger.TRIP_LIST_COLUMN_TIME_TARGET] == Passenger.TIME_TARGET_ARRIVAL)
        ...
        self.modes_df = self.trip_list_df[[Passenger.TRIP_LIST_COLUMN_USER_CLASS,
                                           ...
                                           Passenger.TRIP_LIST_COLUMN_EGRESS_MODE]].set_index(
        ...
        self.modes_df.index.names = [Passenger.TRIP_LIST_COLUMN_USER_CLASS, Passenger.TRIP_LIST_COLUMN_PURPOSE,
        ...
        self.modes_df[PathSet.WEIGHTS_COLUMN_DEMAND_MODE_TYPE] = self.modes_df[
            PathSet.WEIGHTS_COLUMN_DEMAND_MODE_TYPE].apply(lambda x: x[:-5])
        ...
        self.trip_list_df = PathSet.verify_weight_config(self.modes_df, output_dir, routes, capacity_constraint,
                                                         self.trip_list_df)
    ...
    def read_passenger_pathsets(self, pathset_dir, stops, modes_df, include_asgn=True):
        ...
        if Passenger.PF_COL_LINK_TIME in link_cols:
            ...
            pathset_links_df[Passenger.PF_COL_LINK_TIME] = pd.to_timedelta(
                pathset_links_df["%s min" % Passenger.PF_COL_LINK_TIME], unit='m')
        ...
        if Passenger.PF_COL_WAIT_TIME in link_cols:
            ...
            pathset_links_df[Passenger.PF_COL_WAIT_TIME] = pd.to_timedelta(
                pathset_links_df["%s min" % Passenger.PF_COL_WAIT_TIME], unit='m')

        # if simulation results are available
        if Assignment.SIM_COL_PAX_LINK_TIME in link_cols:
            pathset_links_df[Assignment.SIM_COL_PAX_LINK_TIME] = pd.to_timedelta(
                pathset_links_df[Assignment.SIM_COL_PAX_LINK_TIME])
        elif "%s min" % Assignment.SIM_COL_PAX_WAIT_TIME in link_cols:
            pathset_links_df[Assignment.SIM_COL_PAX_LINK_TIME] = pd.to_timedelta(
                pathset_links_df["%s min" % Assignment.SIM_COL_PAX_LINK_TIME], unit='m')

        if Assignment.SIM_COL_PAX_WAIT_TIME in link_cols:
            pathset_links_df[Assignment.SIM_COL_PAX_WAIT_TIME] = pd.to_timedelta(
                pathset_links_df[Assignment.SIM_COL_PAX_WAIT_TIME])
        elif "%s min" % Assignment.SIM_COL_PAX_WAIT_TIME in link_cols:
            pathset_links_df[Assignment.SIM_COL_PAX_WAIT_TIME] = pd.to_timedelta(
                pathset_links_df["%s min" % Assignment.SIM_COL_PAX_WAIT_TIME], unit='m')
        ...
        if "A_id_num" not in pathset_links_df.columns.values:
            pathset_links_df = stops.add_numeric_stop_id(pathset_links_df, id_colname="A_id",
                                                         numeric_newcolname="A_id_num",
                                                         warn=True, warn_msg="read_passenger_pathsets: invalid stop ID",
                                                         drop_failures=False)
        if "B_id_num" not in pathset_links_df.columns.values:
            pathset_links_df = stops.add_numeric_stop_id(pathset_links_df, id_colname="B_id",
                                                         numeric_newcolname="B_id_num",
                                                         warn=True, warn_msg="read_passenger_pathsets: invalid stop ID",
                                                         drop_failures=False)
            ...
            for pathnum in range(pathset.num_paths()):
                ...
                for (state_id, state) in state_list:
                    ...
                    if linkmode == PathSet.STATE_MODE_TRIP and prev_linkmode == PathSet.STATE_MODE_TRIP:
                        FastTripsLogger.warn(
                            "Two trip links in a row... this shouldn't happen. person_id is %s trip is %s\npathnum is %d\nstatelist (%d): %s\n" % (
    ...
    @staticmethod
    def write_paths(output_dir, iteration, pathfinding_iteration, simulation_iteration, pathset_df, links,
                    output_pathset_per_sim_iter, drop_debug_columns, drop_pathfinding_columns):
        """
        ...
        if simulation_iteration < 0:
            ...
            Util.write_dataframe(df=pathset_df,
                                 ...
                                 output_file=os.path.join(output_dir,
                                                          Passenger.PF_LINKS_CSV if links else Passenger.PF_PATHS_CSV),
        ...
        Util.write_dataframe(df=pathset_df,
                             name="pathset_links_df" if links else "pathset_paths_df",
                             output_file=os.path.join(output_dir,
                                                      Passenger.PATHSET_LINKS_CSV if links else Passenger.PATHSET_PATHS_CSV),
                             append=do_append,
                             drop_debug_columns=drop_debug_columns,
                             drop_pathfinding_columns=drop_pathfinding_columns)
    ...
    @staticmethod
    def choose_paths(choose_for_everyone, iteration, pathfinding_iteration, simulation_iteration, pathset_paths_df,
                     pathset_links_df):
        """
        ...
        * :py:attr:`Assignment.CHOSEN_REJECTED` for chosen but then rejected
        ...
        if choose_for_everyone:
            pathset_paths_df[Assignment.SIM_COL_PAX_CHOSEN] = pd.Categorical(
                [Assignment.CHOSEN_NOT_CHOSEN_YET] * len(pathset_paths_df), categories=Assignment.CHOSEN_CATEGORIES,
                ordered=True)
        else:
            ...
            if pathset_paths_df[Assignment.SIM_COL_PAX_CHOSEN].dtype.name != "category":
                ...
                pathset_paths_df[Assignment.SIM_COL_PAX_CHOSEN].cat.set_categories(Assignment.CHOSEN_CATEGORIES,
                                                                                   ordered=True, inplace=True)

            # Otherwise, just choose for those that still need it
            rejected_paths = pathset_paths_df.loc[
        ...
        pathset_paths_df_grouped = pathset_paths_df[[Passenger.TRIP_LIST_COLUMN_PERSON_ID,
                                                     Passenger.TRIP_LIST_COLUMN_PERSON_TRIP_ID,
                                                     Assignment.SIM_COL_PAX_CHOSEN]].groupby(
            [Passenger.TRIP_LIST_COLUMN_PERSON_ID,
             Passenger.TRIP_LIST_COLUMN_PERSON_TRIP_ID]).aggregate("max").reset_index()
        pathset_paths_df_grouped[Assignment.SIM_COL_PAX_CHOSEN] = pd.Categorical(
            pathset_paths_df_grouped[Assignment.SIM_COL_PAX_CHOSEN], categories=Assignment.CHOSEN_CATEGORIES,
            ordered=True)

        # if there's no chosen AND one of the unchosen options is choosable then we can choose
        num_rejected = len(pathset_paths_df_grouped.loc[pathset_paths_df_grouped[
                                                            Assignment.SIM_COL_PAX_CHOSEN] == Assignment.CHOSEN_REJECTED])  # everything is rejected
        num_unchosen = len(pathset_paths_df_grouped.loc[pathset_paths_df_grouped[
        ...
        pax_choose_df = pathset_paths_df_grouped.loc[
        ...
        FastTripsLogger.info(
            "          Have %6d total passenger-trips, with %6d chosen paths, %6d fully rejected and %6d needing a choice" % (
            len(pathset_paths_df_grouped), num_chosen, num_rejected, num_unchosen))
        ...
        paths_choose_df = pathset_paths_df.loc[(pathset_paths_df["to_choose"] == 1) &
                                               (pathset_paths_df[Assignment.SIM_COL_PAX_COST] < PathSet.HUGE_COST) &
                                               (pathset_paths_df[
                                                    Assignment.SIM_COL_PAX_CHOSEN] == Assignment.CHOSEN_NOT_CHOSEN_YET)].copy()
        ...
        chosen_path_df = paths_choose_df[[Passenger.TRIP_LIST_COLUMN_PERSON_ID,
                                          ...
                                          "rand_less"]].groupby([Passenger.TRIP_LIST_COLUMN_PERSON_ID,
                                                                 Passenger.TRIP_LIST_COLUMN_PERSON_TRIP_ID]).idxmax(
            axis=0).reset_index()
        ...
        pathset_paths_df.loc[
            pathset_paths_df["chosen_idx"] == pathset_paths_df.index, Assignment.SIM_COL_PAX_CHOSEN] = CHOSEN_VALUE

        if len(Assignment.TRACE_IDS) > 0:
            FastTripsLogger.debug("choose_path() pathset_paths_df=\n%s\n" % pathset_paths_df.loc[
                pathset_paths_df[Passenger.TRIP_LIST_COLUMN_TRACE] == True].to_string())
        ...
        pathset_links_df[Assignment.SIM_COL_PAX_CHOSEN] = pathset_links_df[Assignment.SIM_COL_PAX_CHOSEN].astype(
            'category')
        pathset_links_df[Assignment.SIM_COL_PAX_CHOSEN].cat.set_categories(Assignment.CHOSEN_CATEGORIES, ordered=True,
                                                                           inplace=True)
    ...
    def get_chosen_links(pathset_links_df, transit_only=False, copy=True):
        ...
        if pathset_links_df[Assignment.SIM_COL_PAX_CHOSEN].dtype.name != "category":
            pathset_links_df[Assignment.SIM_COL_PAX_CHOSEN] = pathset_links_df[Assignment.SIM_COL_PAX_CHOSEN].astype(
                'category')
            pathset_links_df[Assignment.SIM_COL_PAX_CHOSEN].cat.set_categories(Assignment.CHOSEN_CATEGORIES,
                                                                               ordered=True, inplace=True)
