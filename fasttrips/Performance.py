...
class Performance(object):
    ...
    def add_info(self, iteration, pathfinding_iteration, person_id, person_trip_id, perf_dict):
        ...
        self.performance_pf_dict[Performance.PERFORMANCE_PF_COL_TIME_LABELING].append(
            datetime.timedelta(milliseconds=perf_dict[Performance.PERFORMANCE_PF_COL_TIME_LABELING_MS]))
        self.performance_pf_dict[Performance.PERFORMANCE_PF_COL_TIME_ENUMERATING].append(
            datetime.timedelta(milliseconds=perf_dict[Performance.PERFORMANCE_PF_COL_TIME_ENUMERATING_MS]))
    ...
    def write_pathfinding(self, output_dir, append):
        ...
        Util.write_dataframe(performance_df, "performance_df",
                             os.path.join(output_dir, Performance.OUTPUT_PERFORMANCE_PF_FILE), append=append)
    ...
    def write(self, output_dir):
        ...
        performance_df[Performance.PERFORMANCE_COL_STEP_DURATION] = performance_df[
                                                                        Performance.PERFORMANCE_COL_END_TIME] - \
                                                                    performance_df[
                                                                        Performance.PERFORMANCE_COL_START_TIME]

        Util.write_dataframe(performance_df, "performance_df",
                             os.path.join(output_dir, Performance.OUTPUT_PERFORMANCE_FILE), append=False)
