...
class Assignment(object):
    ...
    def read_configuration(config_fullpath):
        ...
        Assignment.NETWORK_BUILD_DATE_START_TIME = datetime.datetime.combine(Assignment.NETWORK_BUILD_DATE,
                                                                             datetime.time())
        ...
        Assignment.TRANSFER_FARE_IGNORE_PATHFINDING = parser.getboolean('pathfinding',
        ...
        if Assignment.PATHFINDING_TYPE not in [Assignment.PATHFINDING_TYPE_STOCHASTIC, \
            ...
            msg = "pathfinding type [%s] not available. Expected values: %s" % (Assignment.PATHFINDING_TYPE, str([
        ...
        if PathSet.OVERLAP_VARIABLE not in PathSet.OVERLAP_VARIABLE_OPTIONS:
            msg = "pathfinding.overlap_variable [%s] not defined. Expected values: %s" % (
                PathSet.OVERLAP_VARIABLE, str(PathSet.OVERLAP_VARIABLE_OPTIONS))
            FastTripsLogger.fatal(msg)
            raise ConfigurationError(config_fullpath, msg)
        if PathSet.USER_CLASS_FUNCTION not in PathSet.CONFIGURED_FUNCTIONS:
            msg = "User class function [%s] not defined.  Please check your function file [%s]" % (
                PathSet.USER_CLASS_FUNCTION, Assignment.CONFIGURATION_FUNCTIONS_FILE)
            FastTripsLogger.fatal(msg)
            raise ConfigurationError(config_fullpath, msg)
    ...
    def process_weight_qualifiers(weights):
        ...
        growth_type[PathSet.WEIGHTS_GROWTH_TYPE] = growth_type[PathSet.WEIGHTS_COLUMN_WEIGHT_NAME].str.extract(
            '((?<=\.)\w+)', expand=False)
        growth_type[PathSet.WEIGHTS_COLUMN_WEIGHT_NAME] = growth_type[PathSet.WEIGHTS_COLUMN_WEIGHT_NAME].str.extract(
            '(\w+(?=\.))', expand=False)
    ...
    def write_configuration(output_dir):
        ...
        parser.set('fasttrips', 'output_passenger_trajectories',
                   'True' if Assignment.OUTPUT_PASSENGER_TRAJECTORIES else 'False')
        parser.set('fasttrips', 'output_pathset_per_sim_iter',
                   'True' if Assignment.OUTPUT_PATHSET_PER_SIM_ITER else 'False')
        ...
        parser.set('fasttrips', 'prepend_route_id_to_trip_id',
                   'True' if Assignment.PREPEND_ROUTE_ID_TO_TRIP_ID else 'False')
        ...
        parser.set('pathfinding', 'transfer_fare_ignore_pathfinding',
                   'True' if Assignment.TRANSFER_FARE_IGNORE_PATHFINDING else 'False')
        parser.set('pathfinding', 'transfer_fare_ignore_pathenum',
                   'True' if Assignment.TRANSFER_FARE_IGNORE_PATHENUM else 'False')

        parser.set('pathfinding', 'user_class_function', '%s' % PathSet.USER_CLASS_FUNCTION)

        parser.set('pathfinding', 'arrive_late_allowed_min',
                   '%f' % (PathSet.ARRIVE_LATE_ALLOWED_MIN.total_seconds() / 60.0))
        parser.set('pathfinding', 'depart_early_allowed_min',
                   '%f' % (PathSet.DEPART_EARLY_ALLOWED_MIN.total_seconds() / 60.0))
    ...
    @staticmethod
    def merge_pathsets(pathfind_trip_list_df, pathset_paths_df, pathset_links_df, new_pathset_paths_df,
                       new_pathset_links_df):
        """
        ...
        FastTripsLogger.debug("merge_pathsets():     pathset_paths_df len=%d head=\n%s" % (
            len(pathset_paths_df), pathset_paths_df.head().to_string()))
        FastTripsLogger.debug("merge_pathsets(): new_pathset_paths_df len=%d head=\n%s" % (
            len(new_pathset_paths_df), new_pathset_paths_df.head().to_string()))
        ...
        FastTripsLogger.debug("merge_pathsets(): new_pathset_links_df len=%d head=\n%s" % (
            len(new_pathset_links_df), new_pathset_links_df.head().to_string()))
        FastTripsLogger.debug("merge_pathsets() dtypes=\n%s" % str(pathset_links_df.dtypes))

        FastTripsLogger.debug("merge_pathsets():     pathfind_trip_list_df len=%d head=\n%s" % (
            len(pathfind_trip_list_df), pathfind_trip_list_df.head().to_string()))
        ...
        new_pathset_paths_df[Assignment.SIM_COL_PAX_CHOSEN] = pd.Categorical(
            [Assignment.CHOSEN_NOT_CHOSEN_YET] * len(new_pathset_paths_df),
        ...
        FastTripsLogger.debug("merge_pathsets():     pathset_paths_df len=%d head=\n%s\ntail=\n%s" % (
            len(pathset_paths_df), pathset_paths_df.head().to_string(), pathset_paths_df.tail().to_string()))
        FastTripsLogger.debug("merge_pathsets():     pathset_links_df len=%d head=\n%s\ntail=\n%s" % (
            len(pathset_links_df), pathset_links_df.head().to_string(), pathset_links_df.tail().to_string()))
    ...
    def number_of_pathsets(pathset_paths_df):
        ...
        """
        return len(
            pathset_paths_df.groupby([Passenger.PERSONS_COLUMN_PERSON_ID, Passenger.TRIP_LIST_COLUMN_PERSON_TRIP_ID]))
        ...
        for iteration in range(1, Assignment.MAX_ITERATIONS + 1):
            ...
            for pathfinding_iteration in range(1, Assignment.MAX_PF_ITERATIONS + 1):
                ...
                FastTripsLogger.info(
                    "***************************** ITERATION %d PATHFINDING ITERATION %d **************************************" % (
                        iteration, pathfinding_iteration))

                if (Assignment.PATHFINDING_TYPE == Assignment.PATHFINDING_TYPE_READ_FILE) and (iteration == 1) and (
                    ...
                    (new_pathset_paths_df, new_pathset_links_df) = FT.passengers.read_passenger_pathsets(output_dir,
                                                                                                         FT.stops,
                                                                                                         FT.routes.modes_df,
                                                                                                         include_asgn=False)
                    ...
                    num_new_paths_found = Assignment.generate_pathsets(FT, pathset_paths_df, veh_trips_df, output_dir,
                                                                       iteration, pathfinding_iteration)
                    (new_pathset_paths_df, new_pathset_links_df) = FT.passengers.setup_passenger_pathsets(iteration,
                                                                                                          pathfinding_iteration,
                                                                                                          FT.stops,
                                                                                                          FT.trips.trip_id_df,
                                                                                                          FT.trips.trips_df,
                    ...
                    Passenger.write_paths(output_dir, iteration, pathfinding_iteration, -1, new_pathset_paths_df, False,
                                          Assignment.OUTPUT_PATHSET_PER_SIM_ITER, not Assignment.DEBUG_OUTPUT_COLUMNS,
                                          False)
                    ...
                    FT.performance.write_pathfinding(output_dir,
                                                     append=((iteration > 1) or (pathfinding_iteration > 1)))
                ...
                # Otherwise, merge with those for whom we already have
                    ...
                    (pathset_paths_df, pathset_links_df) = Assignment.merge_pathsets(
                        FT.passengers.pathfind_trip_list_df, pathset_paths_df, pathset_links_df, new_pathset_paths_df,
                        new_pathset_links_df)

                # if we have new paths, simulate them
                if num_new_paths_found > 0:
                    pathset_paths_df, pathset_links_df = Assignment.merge_prior_choices(pathset_paths_df,
                                                                                        pathset_links_df, success_df)

                    if Assignment.SIMULATION:
                        FastTripsLogger.info(
                            "***************************** ITERATION %d PATHFINDING ITERATION %d *** SIMULATING ***********************" % (
                                iteration, pathfinding_iteration))
                        ...
                        (num_passengers_arrived, pathset_paths_df, pathset_links_df, veh_trips_df) = \
                            Assignment.simulate(FT, output_dir, iteration, pathfinding_iteration, pathset_paths_df,
                                                pathset_links_df, veh_trips_df)
                    else:
                        # if we're not simulating, we can still calculate costs and choose paths
                        FastTripsLogger.info(
                            "***************************** ITERATION %d PATHFINDING ITERATION %d *****CHOOSING PATHS WITHOUT SIMULATING" % (
                                iteration, pathfinding_iteration))
                        FT.performance.record_step_start(iteration, pathfinding_iteration, -1,
                                                         "choosing_without_simulating")
                        (num_passengers_arrived, pathset_paths_df, pathset_links_df) = \
                            Assignment.choose_paths_without_simulation(FT, output_dir, iteration, pathfinding_iteration,
                                                                       pathset_paths_df, pathset_links_df, veh_trips_df)

                FT.performance.record_step_start(iteration, pathfinding_iteration, -1,
                                                 "output_per_pathfinding_iteration")
    ...
    def filter_trip_list_to_not_arrived(trip_list_df, pathset_paths_df):
        ...
        FastTripsLogger.debug("filter_trip_list_to_not_arrived(): trip_list_df len=%d head()=\n%s" % (
            len(trip_list_df), trip_list_df.head().to_string()))
        ...
        chosen_paths_df = pathset_paths_df.loc[
        ...
        trip_list_df_to_return = trip_list_df_to_return.loc[
            pd.isnull(trip_list_df_to_return[Assignment.SIM_COL_PAX_CHOSEN])]
        ...
        FastTripsLogger.debug("filter_trip_list_to_not_arrived(): trip_list_df_to_return len=%d head()=\n%s" % (
            len(trip_list_df_to_return), trip_list_df_to_return.head().to_string()))
    ...
    def generate_pathsets(FT, pathset_paths_df, veh_trips_df, output_dir, iteration, pathfinding_iteration):
        ...
        if iteration == 1 and pathfinding_iteration == 1:
            if Assignment.DEBUG_TRACE_ONLY:
                FT.passengers.trip_list_df = FT.passengers.trip_list_df.loc[
                    FT.passengers.trip_list_df[Passenger.TRIP_LIST_COLUMN_TRACE] == True]
            else:
            ...
            if Assignment.SKIP_PERSON_IDS and len(Assignment.SKIP_PERSON_IDS) > 0:
                FT.passengers.trip_list_df = FT.passengers.trip_list_df.loc[
                    ~FT.passengers.trip_list_df[Passenger.TRIP_LIST_COLUMN_PERSON_ID].isin(Assignment.SKIP_PERSON_IDS)]
        ...
        if Assignment.PATHFINDING_EVERYONE:
            ...
            FT.passengers.pathfind_trip_list_df = Assignment.filter_trip_list_to_not_arrived(FT.passengers.trip_list_df,
                                                                                             pathset_paths_df)
                ...
                for process_idx in range(1, 1 + num_processes):
                    ...
                    process_dict[process_idx] = {
                        "process": multiprocessing.Process(target=find_trip_based_paths_process_worker,
                                                           args=(iteration, pathfinding_iteration, process_idx,
                                                                 Assignment.INPUT_NETWORK_ARCHIVE,
                                                                 Assignment.INPUT_DEMAND_DIR,
                ...
                if num_processes > 1:
                    ...
                    if do_trace:
                        FastTripsLogger.debug(
                            "Tracing assignment of person_id %s and trip %s" % (person_id, person_trip_id))
                    ...
                    if num_paths_sought % info_freq == 0:
                        ...
                        FastTripsLogger.info(
                            " %6d paths sought, %6d paths found of %d paths total.  Time elapsed: %2dh:%2dm:%2ds" % (
                ...
                while done_procs < len(process_dict):
                    ...
                    try:
                        ...
                        if result[1] == "DONE":
                            ...
                            FT.performance.add_info(iteration, pathfinding_iteration, pathset.person_id,
                                                    pathset.person_trip_id, perf_dict)
    ...
    def find_trip_based_pathset(iteration, pathfinding_iteration, pathset, hyperpath, trace):
        ...
        (ret_ints, ret_doubles, path_costs, process_num, pf_returnstatus,
         ...
         bytes_workingset, bytes_privateusage, mem_timestamp) = \
            _fasttrips.find_pathset(iteration, pathfinding_iteration, hyperpath, pathset.person_id,
        ...
        FastTripsLogger.debug(
            "Finished finding path for person %s trip %s" % (pathset.person_id, pathset.person_trip_id))
        pathdict = {}
        row_num = 0
    ...
    def find_passenger_vehicle_times(pathset_links_df, veh_trips_df):
        ...
        if False and len(Assignment.TRACE_IDS) > 0:
            FastTripsLogger.debug("find_passenger_vehicle_times(): input pathset_links_df len=%d\n%s" % \
                                  (len(pathset_links_df), pathset_links_df.loc[
                                      pathset_links_df[Passenger.TRIP_LIST_COLUMN_TRACE] == True].to_string()))
        ...
        intermediate = pd.merge(
            left=pathset_links_df[[Passenger.PERSONS_COLUMN_PERSON_ID, Trip.STOPTIMES_COLUMN_TRIP_ID, 'A_id', 'A_seq',
                                   Passenger.TRIP_LIST_COLUMN_PERSON_TRIP_ID,
        ...
        intermediate = intermediate.rename(columns={
            Trip.STOPTIMES_COLUMN_DEPARTURE_TIME: Assignment.SIM_COL_PAX_BOARD_TIME,
            # transit vehicle depart time (at A) = board time for pax
            Trip.STOPTIMES_COLUMN_ARRIVAL_TIME: Assignment.SIM_COL_PAX_ALIGHT_TIME,
            # transit vehicle arrive time (at B) = alight time for pax
        })
    ...
    def put_passengers_on_vehicles(pathset_links_df, veh_trips_df):
        ...
        passenger_trips_boards = passengers_df.loc[
            passengers_df[Assignment.SIM_COL_PAX_BUMP_ITER].isnull(),  # unbumped passengers
            [Passenger.TRIP_LIST_COLUMN_TRIP_LIST_ID_NUM,
             Trip.STOPTIMES_COLUMN_TRIP_ID_NUM, 'A_id_num', 'A_seq']].groupby(
            [Trip.STOPTIMES_COLUMN_TRIP_ID_NUM, 'A_id_num', 'A_seq']).count()
        ...
        passenger_trips_alights = passengers_df.loc[passengers_df[Assignment.SIM_COL_PAX_BUMP_ITER].isnull(),
                                                    [Passenger.TRIP_LIST_COLUMN_TRIP_LIST_ID_NUM,
                                                     Trip.TRIPS_COLUMN_TRIP_ID_NUM, 'B_id_num', 'B_seq']].groupby(
            [Trip.TRIPS_COLUMN_TRIP_ID_NUM, 'B_id_num', 'B_seq']).count()
        ...
        veh_loaded_df = pd.merge(left=veh_trips_df,
                                 ...
                                 right_index=True,
                                 how='left')
        veh_loaded_df.rename(columns={Passenger.TRIP_LIST_COLUMN_TRIP_LIST_ID_NUM: Trip.SIM_COL_VEH_BOARDS},
                             inplace=True)
        ...
        veh_loaded_df = pd.merge(left=veh_loaded_df,
                                 ...
                                 right_index=True,
                                 how='left')
        veh_loaded_df.rename(columns={Passenger.TRIP_LIST_COLUMN_TRIP_LIST_ID_NUM: Trip.SIM_COL_VEH_ALIGHTS},
                             inplace=True)
        ...
        veh_loaded_df[Trip.SIM_COL_VEH_ONBOARD] = veh_loaded_df[Trip.SIM_COL_VEH_BOARDS] - veh_loaded_df[
            Trip.SIM_COL_VEH_ALIGHTS]
        ...
        veh_loaded_df[Trip.SIM_COL_VEH_OVERCAP] = veh_loaded_df[Trip.SIM_COL_VEH_ONBOARD] - veh_loaded_df[
            Trip.VEHICLES_COLUMN_TOTAL_CAPACITY]
        veh_loaded_df[Trip.SIM_COL_VEH_OVERCAP_FRAC] = 0.0
        veh_loaded_df.loc[veh_loaded_df[Trip.SIM_COL_VEH_BOARDS] > 0, Trip.SIM_COL_VEH_OVERCAP_FRAC] = veh_loaded_df[
                                                                                                           Trip.SIM_COL_VEH_OVERCAP] / \
                                                                                                       veh_loaded_df[
                                                                                                           Trip.SIM_COL_VEH_BOARDS]
    ...
    def flag_missed_transfers(pathset_paths_df, pathset_links_df):
        ...
        * Passenger.PF_COL_PAX_B_TIME
        ...
        pathset_links_df.loc[
            pd.notnull(pathset_links_df[Trip.TRIPS_COLUMN_TRIP_ID]), Assignment.SIM_COL_PAX_ALIGHT_DELAY_MIN] = \
            ((pathset_links_df[Assignment.SIM_COL_PAX_ALIGHT_TIME] - pathset_links_df[
                Passenger.PF_COL_PAX_B_TIME]) / np.timedelta64(1, 'm'))
        ...
        pathset_links_df.loc[
            ...
            pathset_links_df[Assignment.SIM_COL_PAX_BOARD_TIME] - np.timedelta64(24, 'h')
        pathset_links_df.loc[
            pathset_links_df[Assignment.SIM_COL_PAX_ALIGHT_DELAY_MIN] > 22 * 60, Assignment.SIM_COL_PAX_ALIGHT_TIME] = \
            pathset_links_df[Assignment.SIM_COL_PAX_ALIGHT_TIME] - np.timedelta64(24, 'h')
        ...
        if max_alight_delay_min > 0:
            FastTripsLogger.debug("\n%s" % pathset_links_df.sort_values(by=Assignment.SIM_COL_PAX_ALIGHT_DELAY_MIN,
        ...
        pathset_links_df[Assignment.SIM_COL_PAX_B_TIME] = pathset_links_df[Assignment.SIM_COL_PAX_A_TIME] + \
                                                          pathset_links_df[Passenger.PF_COL_LINK_TIME]
        # For trip links, it's alight time
        pathset_links_df.loc[
            pathset_links_df[Passenger.PF_COL_LINK_MODE] == PathSet.STATE_MODE_TRIP, Assignment.SIM_COL_PAX_B_TIME] = \
            pathset_links_df[Assignment.SIM_COL_PAX_ALIGHT_TIME]
        # For access links, it doesn't change from the original pathfinding result
        pathset_links_df.loc[
            pathset_links_df[Passenger.PF_COL_LINK_MODE] == PathSet.STATE_MODE_ACCESS, Assignment.SIM_COL_PAX_A_TIME] = \
            pathset_links_df[Passenger.PF_COL_PAX_A_TIME]
        pathset_links_df.loc[
            pathset_links_df[Passenger.PF_COL_LINK_MODE] == PathSet.STATE_MODE_ACCESS, Assignment.SIM_COL_PAX_B_TIME] = \
            pathset_links_df[Passenger.PF_COL_PAX_B_TIME]
        ...
        pathset_links_df = pd.merge(left=pathset_links_df,
                                    ...
                                    on=[Passenger.TRIP_LIST_COLUMN_PERSON_ID,
                                        ...
                                        Passenger.PF_COL_LINK_NUM])
        pathset_links_df.loc[
            pathset_links_df[Passenger.PF_COL_LINK_MODE] == PathSet.STATE_MODE_TRIP, Assignment.SIM_COL_PAX_A_TIME] = \
            pathset_links_df["new_trip_A_time"]
        pathset_links_df.drop(["new_trip_A_time"], axis=1, inplace=True)

        pathset_links_df[Assignment.SIM_COL_PAX_LINK_TIME] = pathset_links_df[Assignment.SIM_COL_PAX_B_TIME] - \
                                                             pathset_links_df[Assignment.SIM_COL_PAX_A_TIME]

        #: todo: is there a more elegant way to take care of this?  some trips have times after midnight so they're the next day
        #: if the linktime > 22 hours then the trip time is probably off by a day, so it's right after midnight -- back it up
        pathset_links_df.loc[pathset_links_df[Assignment.SIM_COL_PAX_LINK_TIME] / np.timedelta64(1,
                                                                                                 'h') > 22, Assignment.SIM_COL_PAX_B_TIME] = \
            pathset_links_df[Assignment.SIM_COL_PAX_B_TIME] - np.timedelta64(24, 'h')
        pathset_links_df.loc[pathset_links_df[Assignment.SIM_COL_PAX_LINK_TIME] / np.timedelta64(1,
                                                                                                 'h') > 22, Assignment.SIM_COL_PAX_LINK_TIME] = \
            pathset_links_df[Assignment.SIM_COL_PAX_B_TIME] - pathset_links_df[Assignment.SIM_COL_PAX_A_TIME]

        #: if the linktime < -22 hours then the trip time is probably off by a day, so it's right before midnight -- back it up
        pathset_links_df.loc[pathset_links_df[Assignment.SIM_COL_PAX_LINK_TIME] / np.timedelta64(-1,
                                                                                                 'h') < -22, Assignment.SIM_COL_PAX_A_TIME] = \
            pathset_links_df[Assignment.SIM_COL_PAX_A_TIME] - np.timedelta64(24, 'h')
        pathset_links_df.loc[pathset_links_df[Assignment.SIM_COL_PAX_LINK_TIME] / np.timedelta64(-1,
                                                                                                 'h') < -22, Assignment.SIM_COL_PAX_LINK_TIME] = \
            pathset_links_df[Assignment.SIM_COL_PAX_B_TIME] - pathset_links_df[Assignment.SIM_COL_PAX_A_TIME]
        ...
        # new wait time
        pathset_links_df.loc[
            pd.notnull(pathset_links_df[Trip.TRIPS_COLUMN_TRIP_ID]), Assignment.SIM_COL_PAX_WAIT_TIME] = \
            pathset_links_df[Assignment.SIM_COL_PAX_BOARD_TIME] - pathset_links_df[Assignment.SIM_COL_PAX_A_TIME]

        # invalid trips have negative wait time
        pathset_links_df[Assignment.SIM_COL_MISSED_XFER] = 0
        pathset_links_df.loc[pathset_links_df[Assignment.SIM_COL_PAX_WAIT_TIME] / np.timedelta64(1,
                                                                                                 'm') < 0, Assignment.SIM_COL_MISSED_XFER] = 1

        # count how many are valid (sum of invalid = 0 for the trip list id + path)
        pathset_links_df_grouped = pathset_links_df.groupby(
        ...
        pathset_links_df_grouped.loc[
            pathset_links_df_grouped[Assignment.SIM_COL_MISSED_XFER] > 0, Assignment.SIM_COL_MISSED_XFER] = 1
        ...
        pathset_paths_df = pd.merge(left=pathset_paths_df,
                                    right=pathset_links_df_grouped.reset_index()[[Passenger.TRIP_LIST_COLUMN_PERSON_ID,
                                                                                  Passenger.TRIP_LIST_COLUMN_PERSON_TRIP_ID,
                                                                                  Passenger.PF_COL_PATH_NUM,
                                                                                  Assignment.SIM_COL_MISSED_XFER]],
                                    how="left")
        FastTripsLogger.debug("flag_missed_transfers() pathset_paths_df (%d):\n%s" % (
            len(pathset_paths_df), pathset_paths_df.head(30).to_string()))
    ...
    def load_passengers_on_vehicles_with_cap(iteration, pathfinding_iteration, simulation_iteration,
        ...
        while True:  # loop for capacity constraint
            ...
            # bump or board them
            bumpstop_boards[Assignment.SIM_COL_PAX_BOARD_STATE] = pd.Categorical(["boarded"] * len(bumpstop_boards),
    ...
    @staticmethod
    def choose_paths_without_simulation(FT, output_dir, iteration, pathfinding_iteration, pathset_paths_df,
                                        pathset_links_df, veh_trips_df):
        """
        ...
        # Write the pathsets
        Passenger.write_paths(output_dir, iteration, pathfinding_iteration, simulation_iteration, pathset_paths_df,
                              False, Assignment.OUTPUT_PATHSET_PER_SIM_ITER, not Assignment.DEBUG_OUTPUT_COLUMNS,
                              not Assignment.DEBUG_OUTPUT_COLUMNS)
        Passenger.write_paths(output_dir, iteration, pathfinding_iteration, simulation_iteration, pathset_links_df,
                              True, Assignment.OUTPUT_PATHSET_PER_SIM_ITER, not Assignment.DEBUG_OUTPUT_COLUMNS,
                              not Assignment.DEBUG_OUTPUT_COLUMNS)
        ...
        Util.write_dataframe(chosen_links_df, "chosen_links_df", os.path.join(output_dir, "chosenpaths_links.csv"),
                             append=(iteration > 1),
        ...
        Util.write_dataframe(chosen_paths_df, "chosen_paths_df", os.path.join(output_dir, "chosenpaths_paths.csv"),
                             append=(iteration > 1),
                             drop_debug_columns=not Assignment.DEBUG_OUTPUT_COLUMNS,
                             drop_pathfinding_columns=not Assignment.DEBUG_OUTPUT_COLUMNS)
    ...
    def simulate(FT, output_dir, iteration, pathfinding_iteration, pathset_paths_df, pathset_links_df, veh_trips_df):
        ...
        while True:
            FT.performance.record_step_start(iteration, pathfinding_iteration, simulation_iteration,
            ...
            # SIM_COL_PAX_CHOSEN >=0 for chosen paths/path links
            (num_passengers_arrived, num_chosen, pathset_paths_df, pathset_links_df) = Passenger.choose_paths(
                Assignment.PATHFINDING_EVERYONE and simulation_iteration == 0,
                # choose for everyone if we just re-found all paths
                iteration, pathfinding_iteration, simulation_iteration,
                pathset_paths_df, pathset_links_df)

            ######################################################################################################
            FastTripsLogger.info(
                "  Step 5. Put passenger paths on transit vehicles to get vehicle boards/alights/load and assess capacity constraints")
            ...
            if Assignment.OUTPUT_PATHSET_PER_SIM_ITER:
                ...
                Passenger.write_paths(output_dir, iteration, pathfinding_iteration, simulation_iteration,
                                      pathset_paths_df, False,
                                      Assignment.OUTPUT_PATHSET_PER_SIM_ITER, not Assignment.DEBUG_OUTPUT_COLUMNS,
                                      not Assignment.DEBUG_OUTPUT_COLUMNS)
                Passenger.write_paths(output_dir, iteration, pathfinding_iteration, simulation_iteration,
                                      pathset_links_df, True,
                                      Assignment.OUTPUT_PATHSET_PER_SIM_ITER, not Assignment.DEBUG_OUTPUT_COLUMNS,
                                      not Assignment.DEBUG_OUTPUT_COLUMNS)
                # and vehicle trips
                Assignment.write_vehicle_trips(output_dir, iteration, pathfinding_iteration, simulation_iteration,
                                               veh_trips_df)
...
def find_trip_based_paths_process_worker(iteration, pathfinding_iteration, worker_num, input_network_dir,
                                         input_demand_dir, run_config, func_file,
    ...
    FastTripsLogger.info(
        "Iteration %d Pathfinding Iteration %d Worker %2d starting" % (iteration, pathfinding_iteration, worker_num))
    ...
    while True:
        ...
        if (pathset.person_id, pathset.person_trip_id) in Assignment.TRACE_IDS:
            FastTripsLogger.debug(
                "Tracing assignment of person %s trip %s" % (pathset.person_id, pathset.person_trip_id))
            trace_person = True

        try:
            (pathdict, perf_dict) = Assignment.find_trip_based_pathset(iteration, pathfinding_iteration, pathset,
                                                                       hyperpath, trace=trace_person)
